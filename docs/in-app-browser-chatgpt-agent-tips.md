...
YouTube 업로드는 사용자가 공개를 명시하지 않는 한 `private` 상태를 기본값으로 사용한다. 제목, 설명, 태그는 조회수형 문법을 참고하되, 금융 콘텐츠에서는 수익 보장, 원금 보장, 특정 ETF 매수 강요처럼 보이는 표현을 피한다.

업로드 전 체크리스트:

- 썸네일 문구가 모바일에서도 크게 읽히는지 확인한다.
- 제목은 초보자의 질문과 리스크를 같이 담는다.
- 설명에는 핵심 요약, 챕터, 투자 유의 문구를 포함한다.
- 태그는 주제어, 초보자 키워드, 관련 ETF 구조 키워드 순서로 넣는다.
- 사용자가 요청한 경우에만 기존 영상의 썸네일이나 메타데이터를 수정한다.

## 렌더링 품질 정책

같은 이미지가 이어지는 씬 안에서는 자막 구간마다 별도 mp4를 만들어 concat하지 않는다. 배경 이미지 또는 컷어웨이는 씬당 하나의 연속 영상으로 만들고, 자막/타이틀 오버레이만 시간 조건으로 교체한다. 그래야 5초에서 6초로 넘어갈 때 같은 이미지인데 화면이 끊기는 느낌이 줄어든다.

자막 구간 기준:

- 기본 최소 길이: 5초
- 기본 최대 길이: 12초
- 짧은 문장은 이전/다음 문장과 합쳐 자연스럽게 읽히게 한다.
- 최종 렌더 후 `caption-timing.json` 또는 `ffprobe`로 구간 길이를 확인한다.
